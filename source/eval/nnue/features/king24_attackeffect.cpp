...
inline Square King24AttackEffect<AssociatedKing>::GetSquareFromBonaPiece(BonaPiece bp) {
  // 持ち駒の場合
  if (bp < fe_hand_end) {
    return SQ_NB;
  }
...
// Squareのdirty判定
template <Side AssociatedKing>
inline bool King24AttackEffect<AssociatedKing>::IsDirty(Square dirty_sq24[], int dirty_sq24_count, Square sq) {
...
// Effect24::Directの算出
template <Side AssociatedKing>
inline Effect24::Direct King24AttackEffect<AssociatedKing>::CalcDirect(Square sq_king, Square sq) {
...
// 利き数の取得
template <Side AssociatedKing>
inline int King24AttackEffect<AssociatedKing>::GetEffectCount(const Position& pos, Square sq, Color perspective, bool prev_effect) {
  if (sq == SQ_NB) {
    return 0;
  }
  else {
...
template <Side AssociatedKing>
inline Effect24::Direct King24AttackEffect<AssociatedKing>::Inv(Effect24::Direct dir) {
  return Effect24::DIRECT_NB - static_cast<Effect24::Direct>(1) - dir;
}

// 特徴量のインデックスを求める
template <Side AssociatedKing>
inline IndexType King24AttackEffect<AssociatedKing>::MakeIndex(Color perspective, Effect24::Direct dir, int effect) {
...
template <Side AssociatedKing>
void King24AttackEffect<AssociatedKing>::AppendActiveIndices(
...
template <Side AssociatedKing>
void King24AttackEffect<AssociatedKing>::AppendChangedIndices(
  ...
  for (Effect24::Direct dir : Effect24::Direct()) {
    ...
    if (is_ok(sqww)) {
      ...
      // 利き数に変化があった場合
      if (effectCount_prev_2 != effectCount_now_2) {
        removed->push_back(MakeIndex(perspective, dir, effectCount_prev_2));
        added->push_back(MakeIndex(perspective, dir, effectCount_now_2));
      }
    }
...
template class King24AttackEffect<Side::kFriend>;
template class King24AttackEffect<Side::kEnemy>;
